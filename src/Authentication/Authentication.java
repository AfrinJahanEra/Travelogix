...
public class Authentication extends PasswordField {
    ...
    public void saveUserInfo(User user) {
        try (FileWriter writer = new FileWriter(USERS_FILE, true)) {
            writer.write(user.getRole() + ", " + user.getName() + ", " + user.getPhoneNumber() + ", " 
    ...
    public boolean isValidUser(String email, String password) {
        try (BufferedReader reader = new BufferedReader(new FileReader(USERS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] userDetails = line.split(", ");
                if (userDetails.length > 4 && userDetails[3].equalsIgnoreCase(email) && userDetails[4].equals(password)) {
                    return true;
                }
            }
        } catch (IOException e) {
    ...
    public String getPasswordInput() {
        return PasswordField.readPassword("");        
    }
